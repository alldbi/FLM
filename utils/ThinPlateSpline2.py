...
def ThinPlateSpline2(U, source, target, out_size):
    ...
    def _solve_system(source, target):
        ...
        # p_1.set_shape([1, num_point, 1, 3])
        # p_2.set_shape([1, 1, num_point, 3])


        d2 = tf.reduce_sum(tf.square(p_1 - p_2), 3)  # [bn, pn, pn]
        r = d2 * tf.log(d2+1e-6)# [bn, pn, pn]
        eye = tf.eye(num_point)
        r = r + tf.reshape(eye, [1, num_point, num_point])*0.01
    ...
    T, det= _solve_system(source, target)
